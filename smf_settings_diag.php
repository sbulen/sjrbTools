...
$site_title = 'SMF Settings Display';
$db_needed = true;
...
$ui->addChunk('Settings File', function() use ($ui)
{
	...
	foreach($ui->getSettingsFile() as $var => $value)
	{
		if (is_null($value))
			$value = '<em>null</em>';
		elseif ($value === false)
...
$ui->addChunk('$smcFuncs', function() use ($ui)
{
...
{
	global $smcFunc, $db_connection, $db_prefix;   // Must remain globals
