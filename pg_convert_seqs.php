...
$ui->addChunk('Convert Sequences', function() use ($ui)
{
	...
	$sql = "select t.relname as table, c.attname as column, pg_get_expr(d.adbin, d.adrelid) as default
		...
		WHERE pg_get_expr(d.adbin, d.adrelid) like 'nextval(''" . $ui->db->db_prefix . "%'
	...
	foreach($sequences AS $sequence)
	{
		...
		$sql = "SELECT MAX({$sequence['column']}) AS seq_max FROM {$sequence['table']}";
		$result = $ui->db->query($sql);
		$highkey = $ui->db->fetch_assoc($result)['seq_max'];
