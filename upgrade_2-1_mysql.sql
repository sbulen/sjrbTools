...
// temp logging...
$log_text = '=======Processing atts============================= ' . date("F j, Y, g:i a") . "\n";
$log_text .= (empty($custom_av_dir) ? '--no cust av dir--' : 'Cust av dir: ' . print_r($custom_av_dir, true)) . "\n";
file_put_contents('upgr_att.log', $log_text, FILE_APPEND|LOCK_EX);
...
// temp logging...
$log_text = (empty($modSettings['currentAttachmentUploadDir']) ? '--no Curr AttUploadDir--' : 'Curr AttUploadDir: ' . print_r($modSettings['currentAttachmentUploadDir'], true)) . "\n";
$log_text .= (empty($modSettings['attachmentUploadDir']) ? '--no AttUploadDir--' : 'AttUploadDir: ' . print_r($modSettings['attachmentUploadDir'], true)) . "\n";
$log_text .= (empty($modSettings['attachments_21_done']) ? '--no att_21_done--' : 'att_21_done: ' . print_r($modSettings['attachments_21_done'], true)) . "\n";
$log_text .= (empty($upcontext['reprocess_attachments']) ? '--no reprocess flag--' : 'reprocess flag: ' . print_r($upcontext['reprocess_attachments'], true)) . "\n";
file_put_contents('upgr_att.log', $log_text, FILE_APPEND|LOCK_EX);
...
while (!$is_done)
{
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		// temp logging...
		$log_text = (empty($currentFolder) ? '--no curr folder--' : 'Curr folder: ' . print_r($currentFolder, true)) . "\n";
		$log_text .= (empty($row) ? '--no att data--' : 'Att data: ' . print_r($row, true)) . "\n";
		file_put_contents('upgr_att.log', $log_text, FILE_APPEND|LOCK_EX);
		...
		if (empty($row['file_hash']))
		{
			...
			// temp logging...
			$log_text = (empty($oldFile) ? '--no old school old file--' : 'old school old file: ' . print_r($oldFile, true)) . "\n";
			$log_text .= (empty($newFile) ? '--no old school new file--' : 'old school new file: ' . print_r($newFile, true)) . "\n";
			file_put_contents('upgr_att.log', $log_text, FILE_APPEND|LOCK_EX);
		...
		else
		{
			...
			// temp logging...
			$log_text = (empty($oldFile) ? '--no old file--' : 'old file: ' . print_r($oldFile, true)) . "\n";
			$log_text .= (empty($newFile) ? '--no new file--' : 'new file: ' . print_r($newFile, true)) . "\n";
			file_put_contents('upgr_att.log', $log_text, FILE_APPEND|LOCK_EX);
		...
		if ($row['id_member'] != 0)
		{
			...
			// temp logging...
			$log_text = 'Resetting hash for av for member: ' . print_r($row['id_member'], true) . "\n";
			file_put_contents('upgr_att.log', $log_text, FILE_APPEND|LOCK_EX);
		...
		// While were here, do we need to update the mime_type?
